...
public class SyuttaikinController {
	...
	public String syukkin(Model model) {
		...
		if (result.size() < 1) {
			...
			jdbcTemplate.update("INSERT INTO 出退勤 (userID, date, userIDdate,wortime) VALUES(?,?,?,CURTIME());", x, d, z);
	...
	public String taikin(Model model) throws ParseException {
		...
		if (tai.equals("0")) {
			...
			if (isTimeAfterTarget(targetTime)) {
				...
				//退勤時間・残業時間登録
				jdbcTemplate.update("UPDATE 出退勤 SET closetime = CURTIME(), overtime = ? WHERE userIDdate = ?;", lag, z);
				...
				jdbcTemplate.update("UPDATE 出退勤 SET closetime = CURTIME(), overtime = 0 WHERE userIDdate = ?;", z);
	...
	public String syuuryou(Model model) throws ParseException {
		...
		if (beg.equals("0")) {
			...
			if (en.equals("0")) {
				...
				jdbcTemplate.update("UPDATE 出退勤 SET breakends = CURTIME(), breaksum = ? WHERE userIDdate = ?;", breaktime, z);
				List<Map<String, Object>> resultList = jdbcTemplate
						.queryForList(
								"SELECT * FROM 社員 LEFT JOIN 出退勤 ON 社員.userID = 出退勤.userID LEFT JOIN 残業時間 ON 社員.userID = 残業時間.userID WHERE 出退勤.date = ? GROUP BY 社員.userID;",
								d);
